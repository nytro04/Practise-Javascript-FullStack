  ...
  methods: {
    ...
    async loadClient() {
      try {
        ...
        const res = await axios.get(`clients/${this.clientId}`);
        console.log(res);
        if (res.data) {
          const { name, phone, email } = res.data.data.client;

          // this.form = res.data.data.client;
    ...
    async onSubmit() {
      try {
        const invalid = await this.$refs.clientEditForm.validate();
        ...
        if (!invalid) {
          ...
          // console.log(this.selected, "selectde");
          // this.form.providers = this.selected;

          const res = await axios.patch(`clients/${this.clientId}`, this.form);
          ...
          if (res.data) {
            ...
            this.$emit("updatedClient", newProvider);
            this.$refs["edit-client"].hide();
            this.resetModal();
          }
        }
    ...
    resetModal() {
      ...
      //   this.$refs.clientEditForm.reset();
      // });
    },
    ...
    async fetchProviders() {
      try {
        ...
        console.log(res.data.data.providers, "provder");
      } catch (error) {
        console.log(error.response.message, "error");
      }
    },
    ...
    async deleteProvider(id) {
      try {
        ...
        console.log(error.response.message, "delete errro");
      }
    },

    async editProvider(provider) {
      try {
        const res = await axios.patch(provider);
